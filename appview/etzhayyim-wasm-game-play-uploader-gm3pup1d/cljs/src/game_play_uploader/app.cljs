(ns game-play-uploader.app
  "etzhayyim-wasm-game-play-uploader-gm3pup1d appview — reagent + re-frame,
  view built from jp-go-dds (デジタル庁デザインシステム) hiccup.
  ...
  The Svelte page never called the dispatcher, the MCP router, or the
  landing pages under `lp/` — it was pure static markup describing the
  surface's own metadata, and this port adds no campaign functionality
  that was not already there.

  The Svelte scaffold's `routes`/`vars` arrays were stale empty literals
  (`\"routes\": []`, `\"vars\": []`); this port corrects them from the
  component's actual `wrangler.jsonc` (`routes[].pattern` and the `vars`
  map keys), same correction the landed reference this was copied from
  (`cloud-itonami/app-tia`'s `tia-mcp-component/cljs`) already made.

  Backend TypeScript is unaffected by this migration:
  `../src/app.ts` (the dispatcher facade, not deployed — see README) is
...
;; svelte/src/routes/+page.svelte to here. :routes and :vars are corrected
;; from wrangler.jsonc (the Svelte scaffold had stale empty arrays there).
...
(def default-db
  {:title "Game Play Uploader Gm3pup1d"
   ...
   :kind "appview"
   :route-count 2
   :routes ["gm3pup1d.etzhayyim.com/*" "game-play-uploader.etzhayyim.com/*"]
   ...
   :xrpc? true
   :relative-path "appview/etzhayyim-wasm-game-play-uploader-gm3pup1d/cljs/src/game_play_uploader/app.cljs"})
