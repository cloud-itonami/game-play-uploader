(ns game-play-uploader.app-test
  (:require [cljs.test :refer [deftest is testing use-fixtures]]
            [re-frame.core :as rf]
            [re-frame.db :as rf-db]
            [game-play-uploader.app :as app]))
...
(deftest initialize-db-sets-defaults
  (testing ":initialize-db populates the ported +page.svelte `app` object"
    ...
    (is (= "Game Play Uploader Gm3pup1d" @(rf/subscribe [:title])))
    (is (= "etzhayyim-project-game-play-uploader" @(rf/subscribe [:project])))
    (is (= "etzhayyim-wasm-game-play-uploader-gm3pup1d" @(rf/subscribe [:name])))
    (is (= "appview" @(rf/subscribe [:kind])))
    (is (= 2 @(rf/subscribe [:route-count])))
    (is (= ["gm3pup1d.etzhayyim.com/*" "game-play-uploader.etzhayyim.com/*"]
           @(rf/subscribe [:routes])))
    (is (= 9 (count @(rf/subscribe [:vars]))))
    (is (true? @(rf/subscribe [:xrpc?])))
    (is (= "appview/etzhayyim-wasm-game-play-uploader-gm3pup1d/cljs/src/game_play_uploader/app.cljs"
